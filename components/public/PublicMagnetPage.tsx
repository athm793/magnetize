...
import PublicMagnetViewLoader from "@/components/public/PublicMagnetViewLoader";

const RB2B_PIXEL_RE = /^[A-Za-z0-9_-]+$/;

export function getMagnetMetadata(magnet: LeadMagnet): Metadata {
...
export default async function PublicMagnetPage({ magnet }: { magnet: LeadMagnet }) {
  ...
  const rawPixelId = rb2bIntegration ? (rb2bIntegration.config as { pixelId: string }).pixelId : undefined;
  const rb2bPixelId = rawPixelId && RB2B_PIXEL_RE.test(rawPixelId) ? rawPixelId : undefined;
